...
require_relative 'Argentofx_types'
...
class ArgentofxSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ArgentofxHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ArgentofxHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ArgentofxFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ArgentofxUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ArgentofxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ArgentofxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ArgentofxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ArgentofxHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ArgentofxError.new(
        "graphql_error", "ArgentofxSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
