...
module ArgentofxUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ArgentofxResponse.new({})
    ...
    if fetch_err
      ...
      response = ArgentofxResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ArgentofxResponse.new(fetched)
    else
