...
class DollarQuoteEntity
  ...
  def make
    opts = @_entopts.dup
    DollarQuoteEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ArgentofxHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ArgentofxHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.DollarQuote.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ArgentofxHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [DollarQuoteListMatch, Hash, nil] match filter (any subset of
  #   DollarQuote fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
