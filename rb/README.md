...
The Ruby SDK for the Argentofx API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Currency` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = ArgentofxSDK.test(testopts, sdkopts)
```
...
| `DollarQuote` | `(data) -> DollarQuoteEntity` | Create a DollarQuote entity instance. |
...
raise a `ArgentofxError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Argentofx_sdk.rb       -- Main SDK module
...
The main module (`Argentofx_sdk`) exports the SDK class
