...
The Ruby SDK for the Argentofx API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Currency(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Argentofx(nil).load(
  { "id" => "test01" }, nil
)
