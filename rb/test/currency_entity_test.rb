...
class CurrencyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    cfg = ArgentofxConfig.shared_config
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ARGENTOFX_TEST_CURRENCY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    currency_ref01_data_dt0_load_result = Helpers.to_map(currency_ref01_data_dt0_loaded.respond_to?(:data_get) ? currency_ref01_data_dt0_loaded.data_get : currency_ref01_data_dt0_loaded)
    assert !currency_ref01_data_dt0_load_result.nil?
    assert_equal currency_ref01_data_dt0_load_result["id"], currency_ref01_data["id"]
...
def currency_basic_setup(extra)
  ...
  entid_env_raw = ENV["ARGENTOFX_TEST_CURRENCY_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["ARGENTOFX_TEST_CURRENCY_ENTID"])
