...
func TestPrimaryUtility(t *testing.T) {
	...
	t.Run("fetcher-live", func(t *testing.T) {
		...
		liveClient := sdk.NewArgentofxSDK(map[string]any{
	...
	t.Run("fetcher-blocked-test-mode", func(t *testing.T) {
		...
		blockedClient := sdk.NewArgentofxSDK(map[string]any{
			"base": "http://localhost:8080",
			"system": map[string]any{
				"fetch": func(url string, fetchdef map[string]any) (map[string]any, error) {
...
func makeTestCtx(client *sdk.ArgentofxSDK, utility *sdk.Utility, overrides map[string]any) *sdk.Context {
	ctxmap := map[string]any{
		"opname":  "load",
		"client":  client,
		"utility": utility,
...
func makeTestFullCtx(client *sdk.ArgentofxSDK, utility *sdk.Utility) *sdk.Context {
	ctx := makeTestCtx(client, utility, nil)
