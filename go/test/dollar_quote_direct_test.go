...
func TestDollarQuoteDirect(t *testing.T) {
	t.Run("direct-list-dollar_quote", func(t *testing.T) {
		setup := dollar_quoteDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-dollar_quote", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-dollar_quote", func(t *testing.T) {
		setup := dollar_quoteDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-dollar_quote", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type dollar_quoteDirectSetupResult struct {
	client *sdk.ArgentofxSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func dollar_quoteDirectSetup(mockres any) *dollar_quoteDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ARGENTOFX_TEST_DOLLAR_QUOTE_ENTID"]; ok {
		...
		return &dollar_quoteDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &dollar_quoteDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
