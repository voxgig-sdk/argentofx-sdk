...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ARGENTOFX_TEST_LIVE") == "TRUE" ||
		os.Getenv("ARGENTOFX_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ARGENTOFX_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ArgentofxSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ArgentofxError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ArgentofxSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ArgentofxError{Msg: msg, Code: code}
}
