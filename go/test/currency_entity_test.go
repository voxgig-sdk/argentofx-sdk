...
func TestCurrencyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ARGENTOFX_TEST_CURRENCY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		currencyRef01MatchDt0 := map[string]any{
			"id": currencyRef01Data["id"],
		}
		currencyRef01DataDt0Loaded, err := currencyRef01Ent.Load(currencyRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		currencyRef01DataDt0LoadResult := core.ToMapAny(entityData(currencyRef01DataDt0Loaded))
		if currencyRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if currencyRef01DataDt0LoadResult["id"] != currencyRef01Data["id"] {
...
func currencyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ARGENTOFX_TEST_CURRENCY_ENTID")
