...
class ArgentofxSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ArgentofxHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ArgentofxHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ArgentofxFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ArgentofxFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ArgentofxUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ArgentofxHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ArgentofxHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ArgentofxHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ArgentofxError($op . "_allow",
                "ArgentofxSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ArgentofxHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ArgentofxError("graphql_error",
                "ArgentofxSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function DollarQuote($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_dollar_quote === null) {
                $this->_dollar_quote = new DollarQuoteEntity($this, null);
            }
            return $this->_dollar_quote;
        }
        return new DollarQuoteEntity($this, $data);
    }
