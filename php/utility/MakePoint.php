...
// Argentofx SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class ArgentofxMakePoint
{
    public static function call(ArgentofxContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof ArgentofxError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = ArgentofxHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = ArgentofxHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
