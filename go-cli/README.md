...
boru-driven command-line client **and** interactive REPL for the Argentofx
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/argentofx-cli)
make build
...
./argentofx-cli list currency
./argentofx-cli load 1 currency            # {id:1} shorthand
./argentofx-cli load '{id:1}' currency       # explicit match map
./argentofx-cli list dollar_quote

# 5. Override the API base URL for a single call
ARGENTOFX_BASE=https://api.example.com ./argentofx-cli list currency

# 6. No arguments -> interactive REPL
./argentofx-cli
argentofx> list currency
argentofx> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/argentofx-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/argentofx-cli list currency
   ```
...
```sh
./argentofx-cli load 1 currency          # scalar shorthand for {id:1}
./argentofx-cli load '{id:1}' currency     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `argentofx>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `currency`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/argentofx-cli`. |
...
(`./argentofx-cli <expr>`) and interactively (the REPL), and expressions compose the
