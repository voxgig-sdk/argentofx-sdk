// Command Argentofx-cli is a CLI + REPL for the Argentofx SDK,
...
const prompt = "argentofx"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "currency dollar_quote get_root"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ARGENTOFX_APIKEY (api key), ARGENTOFX_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ARGENTOFX_APIKEY carries the API key and
	// ARGENTOFX_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ARGENTOFX_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ARGENTOFX_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewArgentofxSDK(opts)
