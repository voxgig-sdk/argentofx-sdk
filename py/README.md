...
The Python SDK for the Argentofx API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Currency(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Argentofx(None).load(
    {"id": "test01"}, None
)
