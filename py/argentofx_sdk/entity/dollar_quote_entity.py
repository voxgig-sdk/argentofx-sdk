...
from argentofx_sdk.argentofx_types import (
    DollarQuote,
...
class DollarQuoteEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DollarQuoteEntity(self._client, opts)
    ...
    def match_get(self) -> DollarQuote:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DollarQuote:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DollarQuote().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DollarQuote]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DollarQuote().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
