# Argentofx Python SDK Reference

Complete API reference for the Argentofx Python SDK.
...
#### `ArgentofxSDK.test(testopts=None, sdkopts=None)`
...
#### `DollarQuote(data=None)`

Create a new `DollarQuoteEntity` instance. Pass `None` for no initial data.
...
```python
results, err = client.DollarQuote().list({})
```
...
```python
result, err = client.DollarQuote().load({"id": "dollar_quote_id"})
```
...
Create a new `DollarQuoteEntity` instance with the same options.
