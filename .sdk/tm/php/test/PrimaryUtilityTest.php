...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        ArgentofxSDK $client,
        ArgentofxUtility $utility
    ): ArgentofxContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new ArgentofxContext($ctxmap, null);
    ...
    private static function fixctx(ArgentofxContext $ctx, ArgentofxSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ArgentofxError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ArgentofxError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new ArgentofxResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new ArgentofxBaseFeature();
    ...
    {
        $hook_client = ArgentofxSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends ArgentofxBaseFeature {
            public $hook_fn = null;
            public function TestHook(ArgentofxContext $ctx): void
            {
    ...
    {
        $init_client = ArgentofxSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends ArgentofxBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new ArgentofxSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ArgentofxSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ArgentofxSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ArgentofxSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ArgentofxError) ? $result->err->msg : (string)$result->err;
