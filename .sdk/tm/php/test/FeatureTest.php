...
class FtCtrl extends ArgentofxControl
{
...
class FtClient
{
    ...
    private ArgentofxUtility $utility;

    public function __construct(ArgentofxUtility $utility, array $options)
    {
    ...
    public function get_utility(): ArgentofxUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ArgentofxContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ArgentofxContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ArgentofxContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ArgentofxConfig::make_feature($name);
    ...
    private static function buildUrl(ArgentofxSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ArgentofxError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ArgentofxResponse($fetched);
        }
    ...
    private function fail(ArgentofxContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ArgentofxContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
