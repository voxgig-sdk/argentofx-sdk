...
{
    private static function utility(): ArgentofxUtility
    {
        return new ArgentofxUtility();
    }
    ...
    private static function ctx(array $over = []): ArgentofxContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ArgentofxContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ArgentofxOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ArgentofxError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ArgentofxOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ArgentofxMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ArgentofxOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ArgentofxMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ArgentofxOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ArgentofxMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArgentofxSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ArgentofxMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ArgentofxResponse([]),
            'result' => new ArgentofxResult([]),
        ]);
        [, $err] = ArgentofxMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ArgentofxSpec([]),
            'response' => null,
            'result' => new ArgentofxResult([]),
        ]);
        [, $err] = ArgentofxMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ArgentofxSpec(['step' => 's']),
            'response' => new ArgentofxResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ArgentofxResult([]),
        ]);
        [, $err] = ArgentofxMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ArgentofxSpec(['step' => 's']),
            'response' => new ArgentofxResponse(self::resp(200, ['v' => 1])),
            'result' => new ArgentofxResult([]),
        ]);
        [, $err] = ArgentofxMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ArgentofxSpec(['step' => 's']),
            'response' => new ArgentofxResponse(self::resp(200, ['v' => 2])),
            'result' => new ArgentofxResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ArgentofxMakeResponse::call($ctx);
    ...
    {
        $preset = new ArgentofxResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ArgentofxSpec([]),
            'response' => new ArgentofxResponse([]),
            'result' => new ArgentofxResult([]),
        ...
        [$got, $err] = ArgentofxMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ArgentofxResult([])]);
        [, $err] = ArgentofxMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ArgentofxSpec([]), 'result' => null]);
        [, $err] = ArgentofxMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArgentofxSpec(['step' => 's']),
            'result' => new ArgentofxResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ArgentofxOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArgentofxMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArgentofxSpec(['step' => 's']),
            'result' => new ArgentofxResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ArgentofxOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArgentofxMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArgentofxResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ArgentofxSpec([]),
            'result' => new ArgentofxResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ArgentofxMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArgentofxMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ArgentofxError('boom', 'boom');
        $utility->fetcher = function (ArgentofxContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArgentofxSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ArgentofxMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ArgentofxContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ArgentofxMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ArgentofxResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ArgentofxContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArgentofxSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ArgentofxMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ArgentofxResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ArgentofxSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ArgentofxMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ArgentofxMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ArgentofxMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ArgentofxResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ArgentofxDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ArgentofxResult(['ok' => false])]);
        $this->expectException(ArgentofxError::class);
        ArgentofxDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ArgentofxResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ArgentofxMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ArgentofxMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ArgentofxBaseFeature();
        $b = new ArgentofxBaseFeature();
        ArgentofxFeatureAdd::call($ctx, $a);
        ArgentofxFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ArgentofxBaseFeature
    {
        $f = new ArgentofxBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ArgentofxFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ArgentofxContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ArgentofxSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArgentofxPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ArgentofxResponse(['headers' => null]),
            'result' => new ArgentofxResult([]),
        ]);
        ArgentofxResultHeaders::call($ctx);
