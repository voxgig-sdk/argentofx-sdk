...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = ArgentofxBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = ArgentofxSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = ArgentofxSDK.test(nil, nil)
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = ArgentofxSDK.new({
      "base" => "http://localhost:8080",
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = ArgentofxSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = ArgentofxSDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = ArgentofxResult.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = ArgentofxOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = ArgentofxSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = ArgentofxContext.new(ctxmap, nil)
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = ArgentofxResponse.new(resp_map)
      if resp_map["body"]
