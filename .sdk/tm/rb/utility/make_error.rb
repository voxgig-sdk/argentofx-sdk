...
module ArgentofxUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ArgentofxContext.new({}, nil)
    end
    op = ctx.op || ArgentofxOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ArgentofxResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ArgentofxError) ? err.msg : err.to_s
    ...
    sdk_err = ArgentofxError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ArgentofxError)
