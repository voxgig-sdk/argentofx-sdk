...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ARGENTOFX_TEST_LIVE ||
    'TRUE' === process.env.ARGENTOFX_TEST_OVERRIDE
  ) {
  ...
  m.ARGENTOFX_TEST_EXPLAIN = process.env.ARGENTOFX_TEST_EXPLAIN || m.ARGENTOFX_TEST_EXPLAIN
