...
//     (\`new ArgentofxSDK(...)\` / \`ArgentofxSDK.test(...)\` become
//     \`ArgentofxSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"currency":{"test01":{"id":"test01"}},"dollar_quote":{"test01":{"id":"test01"}},"get_root":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ArgentofxSDK(...)\` and any
// \`ArgentofxSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ArgentofxSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ArgentofxSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
