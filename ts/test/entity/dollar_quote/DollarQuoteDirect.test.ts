...
describe('DollarQuoteDirect', async () => {
  ...
  test('direct-load-dollar_quote', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-dollar_quote', setup.live)) return
    ...
    if (setup.live) {
      ...
      const candidateId = listArr[0]?.type ?? listArr[0]?.id
      if (null == candidateId) {
        return // skip: list response shape does not expose load identifier
      }
      params.type = candidateId
  ...
  test('direct-list-dollar_quote', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-dollar_quote', setup.live)) return
    const { client, calls } = setup
