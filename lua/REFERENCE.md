# Argentofx Lua SDK Reference

Complete API reference for the Argentofx Lua SDK.
...
```lua
local results, err = client:DollarQuote(nil):list(nil, nil)
```
...
```lua
local result, err = client:DollarQuote(nil):load({ id = "dollar_quote_id" }, nil)
```
