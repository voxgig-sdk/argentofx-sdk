...
The Lua SDK for the Argentofx API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Currency(nil):list(nil, nil)
...
```lua
local result, err = client:Currency(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Argentofx(nil):load(
  { id = "test01" }, nil
)
