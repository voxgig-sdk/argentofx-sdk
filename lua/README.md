...
The Lua SDK for the Argentofx API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Currency()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── argentofx_sdk.lua    -- Main SDK module
...
The main module (`argentofx_sdk`) exports the SDK constructor
