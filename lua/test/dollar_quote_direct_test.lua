...
describe("DollarQuoteDirect", function()
  it("should direct-list-dollar_quote", function()
    local setup = dollar_quote_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-dollar_quote", setup.live and "live" or "unit")
  ...
  it("should direct-load-dollar_quote", function()
    local setup = dollar_quote_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-dollar_quote", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function dollar_quote_direct_setup(mockres)
