...
[MCP](https://modelcontextprotocol.io) server exposing the Argentofx SDK as
two agent tools — `argentofx_list` and `argentofx_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/argentofx-mcp)
make build

# 2. Provide credentials via the environment
export ARGENTOFX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user argentofx \
  -- /absolute/path/to/argentofx-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./argentofx-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// argentofx_list: first page of records
{ "entity": "currency" }
{ "entity": "currency", "query": { } }

// argentofx_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/argentofx-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user argentofx \
     -- "$PWD"/dist/*/argentofx-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `argentofx_list` and `argentofx_load` tools now appear
   in new sessions. Ask the agent to *"list currency using argentofx"*
   and it calls `argentofx_list` with `{"entity":"currency"}`.
...
```sh
export ARGENTOFX_APIKEY=sk_live_xxx            # API key
export ARGENTOFX_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `argentofx_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `argentofx_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ARGENTOFX_APIKEY` | API key sent with every request. |
| `ARGENTOFX_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

currency | dollar_quote | get_root

### Smoke test via HTTP (raw JSON-RPC)

```sh
./argentofx-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"argentofx_load","arguments":{"entity":"currency","query":{"id":1}}}}'
```
